---
title: 'Get association key by key name'
description: 'Using this api you can get standard / user defined association by key'
---

Using this api you can get standard / user defined association by key

```http
GET https://services.leadconnectorhq.com/associations/key/{key_name}
```

## Autorizacion

<ParamField header="Authorization" type="string" required>
Bearer token generado desde el portal Leadway. Ver [Autenticacion](/authentication).
</ParamField>

<ParamField header="Version" type="string" required default="2021-07-28">
Version de la API.
</ParamField>

## Path parameters

<ParamField path="key_name" type="string" required>
</ParamField>

## Query parameters

<ParamField query="locationId" type="string" required>
</ParamField>

## Respuestas

<Accordion title="200 - Successful response">
<ResponseField name="locationId" type="string" required default='string'>
</ResponseField>

<ResponseField name="id" type="string" required default='ve9EPM428h8vShlRW1KT'>
</ResponseField>

<ResponseField name="key" type="string" required default='student'>
First Objects Association Label (custom_objects.children)
</ResponseField>

<ResponseField name="firstObjectLabel" type="object" required default='student'>
First Objects Association Label (custom_objects.children)
</ResponseField>

<ResponseField name="firstObjectKey" type="object" required default='custom_objects.children'>
First Objects Key
</ResponseField>

<ResponseField name="secondObjectLabel" type="object" required default='Teacher'>
Second Object Association Label (contact)
</ResponseField>

<ResponseField name="secondObjectKey" type="object" required default='contact'>
Second Objects Key
</ResponseField>

<ResponseField name="associationType" type="object" required default='USER_DEFINED'>
Association Type can be USER_DEFINED or SYSTEM_DEFINED
</ResponseField>

```json
{
  "locationId": "string",
  "id": "ve9EPM428h8vShlRW1KT",
  "key": "student",
  "firstObjectLabel": "student",
  "firstObjectKey": "custom_objects.children",
  "secondObjectLabel": "Teacher",
  "secondObjectKey": "contact",
  "associationType": "USER_DEFINED"
}
```
</Accordion>

<Accordion title="400 - Bad Request">
```json
{}
```
</Accordion>

<Accordion title="401 - Unauthorized">
```json
{}
```
</Accordion>

<Accordion title="422 - Unprocessable Entity">
```json
{}
```
</Accordion>


## Ejemplo

<CodeGroup>

```bash cURL
curl -X GET 'https://services.leadconnectorhq.com/associations/key/YOUR_key_name' \
  -H 'Authorization: Bearer YOUR_TOKEN' \
  -H 'Version: 2021-07-28'
```

```javascript Node.js
const response = await fetch('https://services.leadconnectorhq.com/associations/key/YOUR_key_name', {
  method: 'GET',
  headers: {
    Authorization: `Bearer ${process.env.LEADWAY_TOKEN}`,
    Version: '2021-07-28',
  },
});
const data = await response.json();
```

```python Python
import os, requests
response = requests.request(
    'GET',
    'https://services.leadconnectorhq.com/associations/key/YOUR_key_name',
    headers={
        'Authorization': f"Bearer {os.environ['LEADWAY_TOKEN']}",
        'Version': '2021-07-28',
    },
)
data = response.json()
```

```php PHP
<?php
$ch = curl_init('https://services.leadconnectorhq.com/associations/key/YOUR_key_name');
curl_setopt_array($ch, [
    CURLOPT_RETURNTRANSFER => true,
    CURLOPT_CUSTOMREQUEST => 'GET',
    CURLOPT_HTTPHEADER => [
        'Authorization: Bearer ' . getenv('LEADWAY_TOKEN'),
        'Version: 2021-07-28',
    ],
]);
$data = json_decode(curl_exec($ch), true);
```

</CodeGroup>
